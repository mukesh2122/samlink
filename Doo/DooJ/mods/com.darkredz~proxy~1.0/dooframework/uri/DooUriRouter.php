 ...
 * </code></p>
 *
 * <p>If you want a module to be publicly accessed (without using $this->app->getModule() ) , use [module name] ,   eg. /protected/module/forum/PostController.php
 * <code>
 ...
 *
 * @author Leng Sheng Hong <darkredz@gmail.com>
 * @version $Id: DooUriRouter.php 1000 2009-07-7 18:27:22
 * @package doo.uri
 * @since 1.0
 */
class DooUriRouter{
...
//                $this->app->setRawHeader($h);
//			}
//        }
//        $this->app->setRawHeader("Location: $location", true, $code);
//        if($headerAfter!==null){
...
//    }
	...
	private function connect($routes, $subfolder) {
		...
		$type = strtolower($this->app->_SERVER['REQUEST_METHOD']);
		$requestedUri = $this->app->_SERVER['REQUEST_URI'];
		...
		if ($skipNormalRoutes===false) {
			...
			if (isset($possibleRoutes[$requestedUri])===true) {
				...
				if (false === strpos($requestedUri, ':')) {
					...
					$this->app->conf->AUTO_VIEW_RENDER_PATH = $requestedUri;
		...
		if ($skipNormalRoutes===false) {
			foreach($possibleRoutes as $routeKey=>$routeData) {
				...
				$this->app->conf->AUTO_VIEW_RENDER_PATH = $routeKey;
    ...
    public function auto_connect($subfolder='/', $autoroute_alias=null){
        $uri = $this->app->_SERVER['REQUEST_URI'];
        ...
        $module = null;
        if(isset($this->app->conf->MODULES)===true){
            if(in_array($uri[0], $this->app->conf->MODULES)===true){
                $module = $uri[0];
                array_shift($uri);
            }
            //allow modules to be accessed with dash, if the module name consists of underscore
            else if(empty($this->app->route['autoroute_force_dash'])===false){
                $moduleRemUnderscore = explode("\t", str_replace('_', '-', implode("\t", $this->app->conf->MODULES)));
		...
		$this->app->conf->AUTO_VIEW_RENDER_PATH = array($controller_name);
        ...
        if(empty($uri[1])===false){
			...
			$this->app->conf->AUTO_VIEW_RENDER_PATH[] = $uri[1];
		}else{
            $method_name = $method_name_ori = 'index';
			$this->app->conf->AUTO_VIEW_RENDER_PATH[] = 'index';
		}
        ...
        if($autoroute_alias!==null){
            ...
            if(isset($autoroute_alias[$alias])===true){
                ...
                $this->app->conf->AUTO_VIEW_RENDER_PATH[0] = strtolower(preg_replace('/([A-Z])/', '-$1', substr($convertname, 0, -10)));
            }
            else{
                ...
                if( preg_match('/^('. $aliaskey .')\//', '/'.$uridecode.'/', $matchedKey) > 0){
                    ...
                    $this->app->conf->AUTO_VIEW_RENDER_PATH[1] = $method_name;
                }
            }
        }
