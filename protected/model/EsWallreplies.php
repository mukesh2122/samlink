...
class EsWallreplies extends DooSmartModel{
	...
	public function isLiked() {
		...
		$like = Doo::db()->getOne('EsLikes', array(
                    'where' => "ID_WALLITEM = {$this->ID_WALLITEM} AND ID_PLAYER = {$player} AND ReplyNumber = {$this->ReplyNumber}"
                ));
		if($like) {
			if($like->Likes == 1) {
				return true;
