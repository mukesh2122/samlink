<?php if(isset($topReviews) and !empty($topReviews)): ?>
	<!-- Top news start -->
	<div class="list_container">
		...
		<?php $total = count($topReviews); ?>
		<div class="item_list content_shown <?php echo User::isBlockVisible($blockType) ? '' : 'dn'; ?>">
			<?php foreach($topReviews as $key=>$item) {
				$style = '';
				if(!$key) { $style = 'first'; }
                else if($key + 1 == $total) { $style = 'last'; };
				echo $this->renderBlock('reviews/reviewItemLine', array('item' => $item, 'stars' => (5-$key)));
			};?>
