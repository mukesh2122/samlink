...
$SocialRating = PlayerHelper::DisplayRating($player);
$VisitCount = PlayerHelper::DisplayVisits($player);
...
if(isset($adminActions['deleteFriend']) and $adminActions['deleteFriend'] === true && !$noProfileFunctionality) {
	$showAdminFunc = true;
	$funcList[] = '<a class="delete_friend" rel="'.$player->ID_PLAYER.'" href="javascript:void(0)">'.$this->__('Remove player').'</a>';
}
...
if(isset($adminActions['categorizeFriend']) and $adminActions['categorizeFriend'] === true && !$noProfileFunctionality) {
	...
	$query="
		SELECT sn_playerfriendcat.*,sn_players.* ,sn_playerfriendcat_rel.ID_FRIEND
...
<?php
	$listUserIsApproved = MainHelper::IsPlayerApproved($player->ID_PLAYER);
        //$count = MainHelper::nextNumber($player->ID_PLAYER);
...
/*			//Friendcategory banner on avatar
	...
	<div class="list_item_meta">
		...
		<?php if ($listUserIsApproved==1 && $blockStatus==0 && $extBlockStatus==0): ?>
			<h3><a class="list_item_header" href="<?php echo MainHelper::site_url('player/'.$player->URL); ?>"><?php echo $rank. ". " , $nick;?></a></h3>
		<?php else: ?>
		...
		<span class="player_item_location">
			<?php if($location != '-' and trim($location) != ''):?>
				<?php echo $this->__('Lives in [_1]', array($location));?>
                                <p><?php echo $VisitCount; ?><br/>
                                <?php echo $SocialRating; ?></p>
