	...
	if(!empty($categories)): ?>
		<ul class="horizontal_tabs clearfix">
			<li class="<?php echo ($selectedCategory === 0)?'active' : ''; ?>">
				<a href="<?php echo MainHelper::site_url('players/top-list'); ?>" class="icon_link"><i class="games_tab_icon"></i><?php echo $this->__('All'); ?></a>
			</li>
			<?php foreach($categories as $category): ?>
				<li class="<?php echo ($selectedCategory == $category['ID_CATEGORY'])?'active' : ''; ?>">
					<a href="<?php echo MainHelper::site_url('players/top-list/category/' . $category['ID_CATEGORY']); ?>" class="icon_link"><i class="games_tab_icon"></i><?php echo $category['CategoryName']; ?></a>
				</li>
			<?php endforeach; ?>
		</ul>
	<?php endif;
