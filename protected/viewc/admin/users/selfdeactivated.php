<form name="selfdeactivatedform" class="standard_form" method="post" action="#"> 
...
<table cellspacing="0" cellpadding="0" class="table table_bordered table_striped gradient_thead mt10">
	...
	<?php
	$player = User::getUser();
	if(isset($usersSelfdeactivated) and !empty($usersSelfdeactivated)) 
	{
		foreach($usersSelfdeactivated as $user):?>
			<?php if($player->canAccess('Edit user information') === TRUE || strpos(',0,3,4,', ','.$user['ID_USERGROUP'].',') !== FALSE): ?>
				<tr>
					...
					<td class="centered">
						<a href="#" onclick="actionType.value='release';ID_PLAYER.value='<?php echo $user['ID_PLAYER'];?>'; if (confirm('Are you sure?'))document.selfdeactivatedform.submit();"><?php echo $this->__('Release');?></a>
					</td>
				</tr>
			<?php endif; ?>
		<?php endforeach;
