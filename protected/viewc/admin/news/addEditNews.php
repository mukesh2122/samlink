...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#newsOwner").on("change", function() {
            if (this.value !== "0") {
                ajaxCall(this.value, "newsPlatform", "<?php echo $platformText; ?>")
            }
            ;
        });
...
if(!empty($newsItem) && ($newsItem->isBlog == '1')) {
    $prefixOptions[] = array(
        'text' => 'Blog',
        'value' => '2',
        'selected' => '2'
    );
} else if(!empty($newsItem) && ($newsItem->isReview == '1')) {
    $prefixOptions[] = array(
        'text' => 'Review',
        'value' => '3',
        'selected' => '3'
    ...
    if(!empty($prefixes)) {
        foreach($prefixes as $prefix) {
            $prefixOptions[] = array(
                ...
                'value' => $prefix->ID_PREFIX,
                'selected' => $prefix->ID_PREFIX == $selected
            );
        }
    }
}
...
if ($ownerType == 'game') {
    $platformDisplay = 'display: block;';
} else {
    $platformDisplay = "display: none;";
};
...
$EditorData = array(
    ...
    'Elements' => array(
        ...
        array(
            		'type'		=> 'checkbox',
            		'title'		=> $this->__('Ready For Publishing'),
            		'id'			=> 'newsisReady',
            		'name'		=> 'isReady',
            		'checked'	=> (isset($translated) and $translated->isReady == 1)
        )
    )
);
