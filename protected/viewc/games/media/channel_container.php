				...
				foreach($playlists as $playlist){
					...
					if($result){
						for($i = 0, $len = $result->getVideoAmount();$i < $len;$i++){
							$video = "<span class=\"video_title oh\"><a rel=\"iframe\" title=\"\" href=\"".$game->GAME_URL."/media/".MEDIA_CHANNEL_URL."/".$result->getVideoId($i)."\">".DooTextHelper::limitChar(htmlspecialchars($result->getVideoTitle($i)), 75)."</a></span>";
							$video .= "<a class=\"thickbox\" rel=\"iframe\" href=\"".$game->GAME_URL."/media/".MEDIA_CHANNEL_URL."/".$result->getVideoId($i)."\" title=\"".htmlspecialchars($result->getVideoTitle($i))."\">";
