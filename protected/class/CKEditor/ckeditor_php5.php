...
class CKEditor {
	...
	public function editor($name, $value = "", $config = array(), $events = array()) {
		$attr = "";
		foreach($this->textareaAttributes as $key => $val) { $attr.= " " . $key . '="' . str_replace('"', '&quot;', $val) . '"'; };
        ...
        if(!empty($_config)) { $js .= "CKEDITOR.replace('".$name."', ".$this->jsEncode($_config).");"; }
