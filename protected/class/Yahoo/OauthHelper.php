...
class OauthHelper {
    ...
    public function oauth_compute_hmac_sig($http_method, $url, $params, $consumer_secret, $token_secret) {
        ...
        if ($debug) {
            $yglobal->logit("oauth_compute_hmac_sig:DBG:sig:$sig");
        }
        return $sig;
    }
    ...
    public function signature_base_string($http_method, $url, $params) {
        ...
        $yglobals->logit("signature_base_string:INFO:normalized_base_string:$base_string");
    ...
    public function rfc3986_encode($raw_input) {
        if (is_array($raw_input)) {
            return array_map(array($this, __FUNCTION__), $raw_input);
