...
require Doo::conf()->CLASS_PATH . 'Yahoo/YahooGlobals.php';
require Doo::conf()->CLASS_PATH . 'Yahoo/OauthHelper.php';
...
class Yql {
    ...
    public function __construct($token, $secret) {
        ...
        // Call YQL
        $retarr = $this->call_yql(Doo::conf()->OAUTH_CONSUMER_KEY, Doo::conf()->OAUTH_CONSUMER_SECRET, $this->querynum, $this->access_token, $this->access_token_secret, false, true);
        exit(0);
    }
//    $querynum = 1;
...
//    $access_token_secret='o2345w980945353478594867g3454l45lk324wrd';
    ...
    function call_yql($consumer_key, $consumer_secret, $querynum, $access_token, $access_token_secret, $usePost=false, $passOAuthInHeader=true) {
        ...
        $params['oauth_signature'] = $helper->oauth_compute_hmac_sig($usePost ? 'POST' : 'GET', $url, $params, $consumer_secret, $access_token_secret);
        ...
        if ($usePost) {
            $request_url = $url;
            $yglobals->logit("call_yql:INFO:request_url:$request_url");
            $yglobals->logit("call_yql:INFO:post_body:$query_parameter_string");
        ...
        if (!empty($response)) {
            ...
            if ($body) {
                $yglobals->logit("call_yql:INFO:response:");
                print($yglobals->json_pretty_print($body));
            }
            $retarr = $response;
        }
